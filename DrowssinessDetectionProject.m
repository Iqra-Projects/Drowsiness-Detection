%................reading video.....................
obj = VideoReader("C:\Users\US\Downloads\m3");
image = read(obj, 1);
...
figure,imshow(face),title('face detected');
size1 = size(face,1);
size2 = size(face,2);
...
figure,imshow(Eyes),title('Eyes detected');
...
figure,imshow(mouth),title('Mouth detected');


%..........Working on the eyes refrence frame, converting into different color spaces........
Eyes = rgb2ycbcr(Eyes);
figure,imshow(Eyes),title("YCBCR");
Eyes = rgb2gray(Eyes);
figure,imshow(Eyes),title("GRAYSCALE");
val1  = min(Eyes(:));
val1;
counter1=0;
%------------binarization
...
figure,imshow(mouth),title("YCBCR");
mouth = rgb2gray(mouth);
figure,imshow(mouth),title("GRAYSCALE");
val1  = min(mouth(:));
val1; 
counter2=0;
%------------binariztion
...
obj.NumFrames;
array2(obj.NumFrames) = zeros;
array3(int64(obj.NumFrames/15)) = zeros;
for t = 2:1:obj.NumFrames - 1
    t;
    image = read(obj, t);
    FDetect = vision.CascadeObjectDetector;
